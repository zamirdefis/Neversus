					...
					local hrp = v.Character.HumanoidRootPart
						...
						if glowing then
							local team = v.Team.Name
								...
								glowON.OutlineColor = Color3.fromRGB(, InmateOColor[2], InmateOColor[3])
...
local function ColorPickerFuc(buttc, colort, trans , tpart, cleanstate, name, part)
	...
	local function UpdateColor()
		...
		if tpart ~= nil then
			sliderKrastia(trans[1], tpart, Color3.fromRGB(colort[1], colort[2], colort[3]))
		end
	...
	connects[4] = Transp.FocusLost:Connect(function()
		if tonumber(Transp.Text) ~= nil then
			trans[1] = Transp.Text / 100
			cft.BackgroundTransparency = trans[1]
			UpdateColor()
		end
	end)
		...
		elseif v[1] == secType then
			...
			connects[i] = ch.MouseButton1Click:Connect(function()
				if not colorpickeron then
					...
					ColorPickerFuc(chu, v[2], v[5], transppart, true, cleanerstate[1], cleanerstate[2])
				end
			end)
			tableOffset += 1
			horizontal += 8
...
tb_Highlight.MouseButton1Click:Connect(function()
	if tb_HighlightT then
		...
		tb_HighlightT = true
		if not tb_HighlightT then
			glowing = true
		end
		Cleaner("glow", "HumanoidRootPart")
		tb_HighlightT.check.BackgroundColor3 = CheckOnColor
		if UsingEspNow[1] == nil then
			UpdateESP()
		end
