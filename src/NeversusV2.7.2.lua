...
OnBoxAdd = function(Box)
	...
	if library.pointers.VisualsTabCategoryEspDis and library.pointers.VisualsTabCategoryEspDis.value then
		...
		if Character:WaitForChild("HumanoidRootPart") then
			...
			local Tag = AddTagToBox(Box, "up", math.floor((HRP.Position - CHRP.Position).magnitude), library.pointers.VisualsTabCategoryEspAAgeC.value, Enum.Font[library.pointers.VisualsTabCategoryEspAAgeFont.value], library.pointers.VisualsTabCategoryEspAAgeS.value, library.pointers.VisualsTabCategoryEspAAgeTT.value, library.pointers.VisualsTabCategoryEspAAgeTST.value, library.pointers.VisualsTabCategoryEspAAgeSC.value)
			table.insert(AAge, Tag)
			local connect1
			local connect2
			connect1 = run.Heartbeat:Connect(function()
				local n, r = pcall(function() return math.floor((HRP.Position - CHRP.Position).magnitude) end)
				Tag.Text = n and r or "???"
			end)
			connect2 = Tag:GetPropertyChangedSignal("Parent"):Connect(function()
				connect1:Disconnect()
		...
		if library.pointers.VisualsTabCategoryEspDis and library.pointers.VisualsTabCategoryEspDis.value then
			for _, v in ipairs(BoxesStoragePath:GetChildren()) do
				...
				if Character:WaitForChild("HumanoidRootPart") then
					local HRP = Character.HumanoidRootPart
					local CHRP = plr.Character.HumanoidRootPart
					local Tag = AddTagToBox(v, "up", math.floor((HRP.Position - CHRP.Position).magnitude), library.pointers.VisualsTabCategoryEspAAgeC.value, Enum.Font[library.pointers.VisualsTabCategoryEspAAgeFont.value], library.pointers.VisualsTabCategoryEspAAgeS.value, library.pointers.VisualsTabCategoryEspAAgeTT.value, library.pointers.VisualsTabCategoryEspAAgeTST.value, library.pointers.VisualsTabCategoryEspAAgeSC.value)
					table.insert(AAge, Tag)
					local connect1
					local connect2
					connect1 = run.Heartbeat:Connect(function()
...
VisualsTabCategoryEsp:AddDropdown("Postfix Type", {"Without Postfix", "Simple Postfix", "Full Postfix"}, "Simple Postfix", "VisualsTabCategoryEspAAgePfx", function(val)
...
VisualsTabCategoryEsp:AddSlider("Names Size", {0, 50, 15, 0.01, ""}, "VisualsTabCategoryEspNamesS", function()
	RemoveTags(true)
end)

VisualsTabCategoryEsp:AddSlider("Display Names Size", {0, 50, 15, 0.01, ""}, "VisualsTabCategoryEspDNamesS", function()
	RemoveTags(true)
end)

VisualsTabCategoryEsp:AddSlider("Account Age Size", {0, 50, 15, 0.01, ""}, "VisualsTabCategoryEspAAgeS", function()
	RemoveTags(true)
end)

VisualsTabCategoryEsp:AddSlider("User Id Size", {0, 50, 15, 0.01, ""}, "VisualsTabCategoryEspUserIdS", function()
	RemoveTags(true)
end)
