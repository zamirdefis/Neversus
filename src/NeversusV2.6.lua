...
local function _ESP(c, pl)
	...
	connect2 = pl:GetPropertyChangedSignal("Team"):Connect(function()
		if hl then
			c.hl.FillColor = pl.TeamColor.Color
			c.hl.OutlineColor = pl.TeamColor.Color
		end
		if tagtext then
			tagtext.Color = pl.TeamColor
		end
	end)
end

VisualsTabCategoryEsp:AddToggle("Enabled", false, "VisualsTabCategoryEspEnabled", function(val)
	if val then
		run:BindToRenderStep("EspLoop", Enum.RenderPriority.Camera.Value + 1, function()
		...
		for i,v in ipairs(plrs:GetPlayers()) do
			if v ~= plr then
				...
				cOnn1 = v.CharacterAdded:Connect(function(chr)
					_ESP(chr, v)
				end)
			end
		end
		cOnn2 = plrs.PlayerAdded:Connect(function(player)
		...
		p
		run:UnbindFromRenderStep("EspLoop")
	end
end)
