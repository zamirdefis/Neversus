...
VisualsTabCategoryCamera:AddKeybind("Spectator Mode", Enum.KeyCode.V, "VisualsTabCategorySpecModeKB")

local VisualsTabCategorySelf = VisualsTab:AddCategory("Self", 1)

local function AddFakePartsOnCharacterForMC(Character)
	local FakeParts = Instance.new("Folder")
	FakeParts.Parent = Character
	FakeParts.Name = "FakeParts"
	for _, MeshPart in ipairs(Character:GetChildren())do
		if (MeshPart:IsA("Part")) then
			...
			FakePart.Name = "Fake_" .. MeshPart.Name
...
local function SetSelfChams(Character)
	local FakeParts = AddFakePartsOnCharacterForMC(Character)
	for _, v in ipairs(FakeParts:GetDescendants()) do
		if v:IsA("BasePart") then
			v.Material = library.pointers.VisualsTabCategorySelfSCMaterial and Enum.Material[library.pointers.VisualsTabCategorySelfSCMaterial.value] or Enum.Material.ForceField
		end
	end
end
...
VisualsTabCategorySelf:AddDropdown("Material", Materials, "ForceField", "VisualsTabCategorySelfSCMaterial")

game.Players.LocalPlayer.CharacterAdded:Connect(function(ch)
	if library.pointers.VisualsTabCategorySelfSChams.value then
		local connect
