	...
	if val then
		...
		Flyloop = run.RenderStepped:Connect(function()
			if IsAlive(plr) then
				...
				plr.Character.HumanoidRootPart.Velocity = velocity + Vector3.new(0, 2.95, 0)
...
MiscTabCategoryFly:AddSlider("Speed", {1, 500, 0, 1, ""}, "MiscTabCategoryFlySpeed")
...
MiscTabCategoryFly:AddToggle("No Clip", false, "MiscTabCategoryFlyNoClip", function(val)
	...
	elseif NoClipConnect and IsAlive(plr) then
