...
local function ESPCleaner(val)
	...
	elseif val == 2 then
		if #EspList.Boxes > 0 then
			for _, v in ipairs(EspList.Boxes) do
				v:Destroy()
			end
		end
	end
...
local function AddBox(Model, IsPlayer, Thickness, Color, LineJoinMode, Transparency, FillColor, FillTransparency)
	...
	run:BindToRenderStep("BoxRender", Enum.RenderPriority.Camera.Value + 1, function()
	...
	Box.Destroying:Connect(function()
		run:UnbindFromRenderStep("BoxRender")
		return
	end)
	return Box
end
...
VisualsTabCategoryEsp:AddToggle("Boxes", false, "VisualsTabCategoryEspBoxes", function(val)
	if val then
		for i,v in ipairs(plrs:GetPlayers()) do
			if v ~= plr then
				if v.Character then
					local Box = AddBox(v.Character)
					if Box then
						table.insert(EspList.Boxes, Box)
					end
				end
				cOnn1 = v.CharacterAdded:Connect(function(chr)
					local Box = AddBox(chr)
					if Box then
						table.insert(EspList.Boxes, Box)
					end
				end)
